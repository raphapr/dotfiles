...
function __kube_ps_update_cache
  ...
  for conf in (string split ':' "$kubeconfig")
    if /bin/test -r "$conf"
      echo (stat -c %Y $conf) >> /home/raphael/test.log
      echo "current: $__kube_ps_timestamp" >> /home/raphael/test.log
...
function __kube_prompt
  ...
  echo -n -s (set_color cyan)"⎈"(set_color white)"($__kube_ps_context|$__kube_ps_namespace)"(set_color normal)" "
end
